...
// Cheap, legible retrieval: rank sources by term overlap with the question and
// take the top few. The grounded answerer and verifier then work over these.
export function retrieve(question: string, k = 4): Source[] {
  ...
  const chosen = (withHits.length ? withHits : scored).slice(0, k).map((x) => x.s);
  return chosen;
}
